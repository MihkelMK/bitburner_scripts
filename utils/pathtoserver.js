...
export async function main(ns) {
  ...
  if (ns.args.length < 2) {
    ns.tprint("Usage: run find-path.js [target-hostname] [scan-range]");
    return;
  }
  
  const targetHost = ns.args[0];
  const scanRange = parseInt(ns.args[1]);
  
  if (isNaN(scanRange)) {
    ns.tprint("Error: scan-range must be a number");
    return;
  }
 ...
 */
function findOptimalPath(ns, start, target) {
