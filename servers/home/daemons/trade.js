import { disable_logs, formatCurrency, formatNumber, notify } from "/helpers/cli.js"
...
function sortAndFilterSymbols(tix) {
  const filteredSymbols = tix.getSymbols()
    ...
    .filter(sym => { // check if we didn't over buy this symbol
      ...
      return tix.getMaxShares(sym) * MAX_STOCK_OWNED_PERCENT > Math.max(shares, sharesShort)
    })
  return filteredSymbols
    .sort((a, b) => getSymbolPoint(tix, b) - getSymbolPoint(tix, a))
}
...
export async function main(ns) {
  disable_logs(ns, ["getServerMoneyAvailable", "sleep", "stock.sellStock", "stock.buyStock"]);
  const tix = ns.stock;
  ...
  while (true) {
    ...
    for (let i in owned) {
      ...
      if (shares > 0 && forecast - MIN_EXIT_FORECAST_PERCENT <= 0.5) {
        ...
        if (sellPrice > 0) {
          notify(ns, `Sell ${formatNumber(shares)} x ${sym} for ${formatCurrency(sellPrice * shares)}.`, "trade")
        }
      }
    }
    ...
    if (canBuy(money)) {
      ...
      for (let i in buyCandidates) {
        ...
        if (getSymbolPoint(tix, sym) > 0) {
          const amountToBuy = tix.getMaxShares(sym) * MAX_STOCK_OWNED_PERCENT - shares - sharesShort;
          const amountToAfford = Math.min(amountToBuy, Math.floor(money / tix.getAskPrice(sym)));

          if (amountToAfford > 0) {
            const purchasePrice = tix.buyStock(sym, amountToAfford);

            if (purchasePrice > 0) {
              notify(ns, `Buy ${formatNumber(amountToAfford)} x ${sym} for ${formatCurrency(purchasePrice * amountToAfford)}.`, "trade")
            }
          }
        }
      }
