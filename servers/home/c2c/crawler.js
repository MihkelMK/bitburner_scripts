...
export async function main(ns) {
  disable_logs(ns, ["scan", "run", 'getServerRequiredHackingLevel', 'getHackingLevel', 'getServerNumPortsRequired', "sleep", "nuke", "singularity.installBackdoor", 'brutessh', 'ftpcrack', 'httpworm', 'sqlinject', 'relaysmtp'])
  while (true) {
    let hacks_dict = {
    ...
    let total_hacks = Object.entries(hacks_dict).filter(([name, exists]) => exists).length;
    ...
    while (i < servers.length) {
      ...
      if (!serverObj.purchasedByPlayer && serverObj.requiredHackingSkill <= ns.getHackingLevel() && !IGNORE.includes(server) && serverObj.numOpenPortsRequired <= total_hacks) {
