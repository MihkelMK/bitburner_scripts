import { TARGET_PORT } from '../helpers/ports';
...
function calculate_score(data, stage) {
  ...
  switch (stage) {
    ...
    case 'mid':
      ...
      score =
        (moneyMax * Math.pow(growth, 2)) / (minSecurity * Math.sqrt(hackTime));
...
function find_targets(ns, servers, count, stage) {
  ...
  for (let i = 0; i < Math.min(5, analyzed_servers.length); i++) {
    ...
    ns.print(
      `${t.hostname}: Score=${t.score.toFixed(2)}, $=${ns.formatNumber(t.data.money.max)}, Sec=${t.data.security.min.toFixed(2)}, Growth=${t.data.growth}`
    );
  }
...
export async function main(ns) {
  ...
  if (args.help || args._.length < 1) {
    ns.tprint(
      'Finds n most profitable targets to hack in early/mid/late STAGE of the game.'
    );
