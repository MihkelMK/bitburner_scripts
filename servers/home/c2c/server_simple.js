import { STATE_PORT, GOAL_PORT, TARGET_PORT, HOME_RESERVE_PORT } from "/helpers/ports.js"
import { disable_logs, notify, formatNumber } from "/helpers/cli.js"
...
const COMMANDS = {
  ...
  weaken: {
    src: "weaken.js", mult: 0.25, targeted: true
  },
  ddos: { src: "grow.js", targeted: true },
  share: { src: "share_ram.js", targeted: false }
}
...
/** @param {NS} ns */
export function c2c_setup(ns, server, target, availableRam, additional) {
  if (!additional) {
...
/** @param {NS} ns */
function optimize_server_allocation(ns, server, reserved_on_home, processes) {
  try {
    ...
    const currentThreadsTotal = {};
    const currentTargets = [];

    const results = structuredClone(threadsBase);
    ...
    Object.entries(currentThreadsTotal).forEach(([target, currentThreads]) => {
      ...
      try {
        ...
        notify(ns, `Optimized ${server} | +${mostDeficientTask}[${additionalThreads}] @${target}`);
      } catch (error) {
...
function enforceHomeReservation(ns, c2c_state) {
  ...
  const nonC2CRam = usedRam - getRAMUsedByC2C(ns, "home");
  ...
  if (currentlyFree < c2c_state.reserved_on_home) {
    ...
    notify(ns, `Home reservation violated! Need to free ${needToFree.toFixed(2)}GB of RAM`, "c2c", "warning");
    ...
    for (const proc of c2cProcesses) {
      ...
      if (targetServer && taskType && c2c_state.allocations[targetServer]) {
        ...
        notify(ns, `Killed ${taskType} process (${proc.threads} threads) targeting ${targetServer} to free ${procRam.toFixed(2)}GB`, "c2c", "warning");
      } else {
        notify(ns, `Killed process ${proc.filename} (${proc.threads} threads) to free ${procRam.toFixed(2)}GB`, "c2c", "warning");
      }
...
/** @param {NS} ns */
function hack_setup(ns, c2c_state, server) {
  // Choose 3 random targets, weighted by timeToHack/maxMoney
  const targets = Array(3).fill(0).map(() => weighted_random(c2c_state.targets, c2c_state.targets.map(t => { const data = c2c_state.allocations[t.hostname].data; return data.time / data.money.max })));
  ...
  let allocatedThreads = { hack: 0, grow: 0, weaken: 0 }

  const availableRam = getFreeRAM(ns, server, c2c_state.reserved_on_home);
  ...
  if (availableRam < 64) {
    ...
    for (let i in targets) {
      ...
      const targetThreads = c2c_setup(ns, server, target.hostname, ramPerTarget, additional)
...
export async function main(ns) {
  disable_logs(ns, ["scan", "exec", "scp", "killall", "kill", 'getServerRequiredHackingLevel', 'getHackingLevel', 'getServerNumPortsRequired', 'getServerUsedRam', 'getServerMaxRam', 'sleep'])
      ...
      if (!useless.includes(server) && ns.hasRootAccess(server)) {
        ...
        if (c2c_state.goal === "ddos" && !c2c_state.ddos.includes(server)) {
          ...
          if (threads <= 0) {
            ...
            notify(ns, server + " | ddos[" + threads + "] @ " + target.hostname, "c2c", "success");
          ...
          if (!isAllocated) {
            ...
            if (processes.length === 0) {
              ...
              const { targets, threads } = optimize_server_allocation(ns, server, c2c_state.reserved_on_home, processes);

              if (targets) {
                currTargets = targets.map((name) => c2c_state.targets.find((target) => target.hostname === name))
              }
            ...
            if (!allocatedThreads) {
              notify(`${server} already at capacity`)
              i++;
