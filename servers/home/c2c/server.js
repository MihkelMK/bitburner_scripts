...
function hack_setup(ns, c2c_state, server) {
  ...
  const targets = Array(3)
    .fill(0)
    .map(() =>
      weighted_random(
        c2c_state.targets,
        c2c_state.targets.map((t) => {
          const data = c2c_state.allocations[t.hostname].data;
          return data.time / data.money.max;
        })
      )
    );
