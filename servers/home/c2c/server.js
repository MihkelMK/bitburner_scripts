...
function disable_logs(ns) {
  var logs = ["scan", "exec", "scp", "killall", "kill", 'getServerRequiredHackingLevel', 'getHackingLevel', 'getServerNumPortsRequired', 'getServerGrowth', 'getServerUsedRam', 'getServerMaxMoney', 'getServerMaxRam', 'getServerMoneyAvailable', 'getServerSecurityLevel', 'getServerMinSecurityLevel', 'sleep']
  for (var i in logs) {
...
function printTargetOptimizationStats(ns, targets) {
  try {
    ...
    for (const target of targets) {
      ...
      const moneyStatus = getStatusIndicator(performance.needsMoreGrow, performance.needsLessHack);
      const securityStatus = getStatusIndicator(performance.needsMoreWeaken, false);
    ...
    // Calculate total column widths
    const moneyColWidth = Math.max((columnWidths.moneyInfo + columnWidths.growPercent + columnWidths.status), columnWidths.moneyTitle);
    const securityColWidth = Math.max((columnWidths.securityInfo + columnWidths.weakenPercent + columnWidths.status), columnWidths.securityTitle);
    const hackColWidth = Math.max((columnWidths.hackPercent + columnWidths.status), columnWidths.hackTitle);
...
export function c2c_setup(ns, server, target, reserved_on_home) {
  ...
  if (sets <= 0) {
    ...
    if (currentSecurity > minSecurity + 5) {
      ...
      notify(ns, `${server} | High security, using ${weakenThreads} weaken threads`);
    } else {
      // Normal behavior - prioritize grow
      growThreads = Math.floor(availableRam / scriptRams.grow);
      notify(ns, `${server} | Not enough RAM for even set, using ${growThreads} grow threads`);
    }
  } else {
  ...
  notify(ns, server + " | " + "g[" + formatNumber(growThreads) + "] " + "w[" + formatNumber(weakenThreads) + "] " + "h[" + formatNumber(hackThreads) + "] " + (target ? "@" + target : ""));
...
function optimize_server_allocation(ns, server, target, reserved_on_home) {
  try {
    ...
    try {
      ns.exec(SCRIPTS_DIR + COMMANDS[mostDeficientTask].src, server, additionalThreads, target, randomDelay);
...
function calculateOptimalRatios(ns, target) {
  try {
    ...
    if (securityExcess > 10) {
      ...
      notify(`High security detected on ${target} (excess: ${securityExcess.toFixed(1)}). ` +
...
function calculateRatiosWithFormulas(ns, target) {
  ...
  const growThreads = Math.max(1, Math.ceil(ns.formulas.hacking.growThreads(server, player, server.moneyMax, growthNeeded)));
...
function calculateServerValue(ns, hostname) {
  ...
  return (moneyScore * moneyWeight) + (securityScore * securityWeight) + (growthScore * growthWeight);
}
...
function reassignServers(ns, c2c_state) {
  ...
  for (const server of allServers) {
    if (server !== "home" && ns.hasRootAccess(server) && ns.getServerMaxRam(server) > 0) {
      ns.killall(server);
    }
  }

  // Home needs special handling - only kill C2C scripts
  if (c2c_state.hack.includes("home") || c2c_state.grow.includes("home") ||
    ...
    while (i < servers.length) {
      ...
      if (!useless.includes(server) && ns.hasRootAccess(server)) {
        ...
        if (c2c_state.goal === "ddos" && !c2c_state.ddos.includes(server)) {
          ...
          const threads = Math.floor(getFreeRAM(ns, server, c2c_state.reserved_on_home) / 4);
          ...
          if (!isAllocated) {
            ...
            const target = selectWeightedTarget(ns, c2c_state.targets, targetUsage);

            targetUsage[target.hostname] = (targetUsage[target.hostname] || 0) + 1;
            ...
            notify(ns, `Optimized ${server} | ${hackAdded ? '+h[' + formatNumber(allocatedThreads.hack) + '] ' : ''}${growAdded ? '+g[' + formatNumber(allocatedThreads.grow) + '] ' : ''}${weakenAdded ? '+h[' + formatNumber(allocatedThreads.weaken) + '] ' : ''}@${target.hostname}`);
          }
